...
function getConfig(param) {
  return lumine.config.get(`cursor-history.${param}`);
}
...
module.exports = class History {
  ...
  constructor() {
    ...
    this.configObserver = lumine.config.observe(
  ...
  isJumpableEntry(entry, editor) {
    ...
    if (getConfig("searchAllPanes")) {
      ...
      return entry.URI || lumine.workspace.getActivePane().getItems().includes(entry.editor);
    }
  }
  ...
  // Replacing an editor's whole buffer can move several markers to the same position.
  // See #36 for detail.
  ...
  async jump(editor, direction, editorToFind) {
    ...
    let landingEditor;
    if (entry.editor?.isAlive()) {
      const pane = lumine.workspace.paneForItem(entry.editor);
      if (!pane) {
        throw new Error(`No pane found for ${entry.URI || "untitled editor"}`);
      }
      pane.activate();
    ...
    if (!landingEditor) {
      landingEditor = await lumine.workspace.open(entry.URI, {
        pending: getConfig("openInPendingState"),
        searchAllPanes: getConfig("searchAllPanes"),
      });
    }

    this.land(landingEditor, entry, entry.URI ? entry.URI === fromURI : entry.editor === editor);
